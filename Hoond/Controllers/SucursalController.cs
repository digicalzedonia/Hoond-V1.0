...
namespace Hoond.Controllers
{
    ...
    public class SucursalController : Controller
    {
        ...
        public ActionResult Index(string cmbCampo, string cmbPaginado, string sortField, string txtBusqueda, string msgError, int? page)
        {
            ...
            int Per = DatComun.TipoPer(User.idRol, "Sucursal", "Index");
            if (Per == 0)
            ...
            var Query = db.Sucursal.Include(s => s.Empresa).Include(s => s.Usuario).Include(s => s.TipoSucursal);
			if(!User.isAdmin)
        ...
        public ActionResult Create()
        {
           if (Session["user"] == null)
                return RedirectToAction("Login", "Login");
            UserLogin User = (UserLogin)Session["user"];
            int Per = DatComun.TipoPer(User.idRol, "Sucursal", "Create");
            if (Per == 0)
                return RedirectToAction("Index", new { msgError = app_GlobalResources.Content.msg_AccesoDenegado });
            else if (Per == 1)
                return RedirectToAction("Index", new { msgError = app_GlobalResources.Content.msg_SinPrivilegio });
            ViewBag.idTipoSucursal = new SelectList(db.TipoSucursal.Where(e => e.activo == true && e.idEmpresa == User.idEmpresa), "idTipoSucursal", "descTipoSucursal");
            return View();
        }
        ...
        public ActionResult Create(Sucursal sucursal)
       {
            ...
            {
                sucursal.idSucursal  = DatComun.GetId(1);
                sucursal.idUsuario_alta  = User.idUsuario;
                sucursal.idEmpresa = User.idEmpresa;
                sucursal.fecha_alta = DateTime.Now;
            ...
            ViewBag.idTipoSucursal = new SelectList(db.TipoSucursal.Where(e => e.activo == true && e.idEmpresa == User.idEmpresa), "idTipoSucursal", "descTipoSucursal", sucursal.idTipoSucursal);
            return View(sucursal);
        }
        ...
        public ActionResult Edit(Sucursal sucursal)
        {
            ...
            if (ModelState.IsValid)
            {
                DatComun.CreateLog("Sucursal", "U", sucursal.idSucursal, User.idUsuario);
        ...
        public ActionResult DeleteConfirmed(decimal id)
        {
            ...
            UserLogin User = (UserLogin)Session["user"];
            Sucursal sucursal = db.Sucursal.Find(id);
            DatComun.CreateLog("Sucursal", "D", id, User.idUsuario);
        ...
        public ActionResult DeleteM(decimal[] ids)
        {
            ...
            if (ids != null)
            {
                ...
                var Query = db.Sucursal.Include(s => s.Empresa).Include(s => s.Usuario).Include(s => s.TipoSucursal).Where(q => id.Contains(q.idSucursal));
        ...
        public ActionResult DeleteConfirmedM(decimal[] ids)
        {
            ...
            if (this.IsCaptchaValid(string.Empty))
            {
                ...
                {
                    Item = db.Sucursal.Find(ids[i]);
                    DatComun.CreateLog("Sucursal", "D", ids[i], User.idUsuario);
                    Item.activo = false;
                    db.SaveChanges();
                }
                return RedirectToAction("Index");
